...
def get_admin_dashboard_stats() -> Dict[str, Any]:
    ...
    # 1. Fetch all registered members
    ...
    # Unique members who have at least one attendance row
...
def get_admin_participants(
    ...
    for m in members:
        ...
        # Free-text search across name, email, agent ID and college
        if sq:
...
def verify_qr_token_server(qr_token: str) -> Dict[str, Any]:
    ...
    # Try passport_token, then row id, then agent_id
...
def execute_server_checkin(
    ...
    # Fallback response when the insert did not confirm
    return {
        "success": True,
        "status": "CHECKED_IN_SUCCESS",
