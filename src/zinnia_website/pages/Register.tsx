...
import { ArrowRight, AlertCircle, Users, User, Plus, Trash2, Shield } from 'lucide-react';
...
export const WebsiteRegisterPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!leader.name || !leader.email || !leader.phone || !college) {
      setError('Please provide all mandatory team leader credentials and college name.');
      return;
    }

    if (registeredEvents.length === 0) {
      setError('Please select at least one symposium event.');
      return;
    }
    ...
    // Check all added members have required fields
    for (let i = 0; i < members.length; i++) {
      const m = members[i];
      if (!m.name.trim() || !m.email.trim() || !m.phone.trim()) {
        setError(`Please fill in all details (name, email, phone) for Team Member ${i + 2}.`);
        return;
      }
    }
      ...
      >
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            {registeredEvents.length > 0 && (
              <span className="text-[11px] text-cyan-400 bg-cyan-950/80 px-2.5 py-0.5 rounded-full border border-cyan-500/40">
                Team Size: {minTeamSize} - {maxTeamSize} Member(s)
              </span>
            )}
          </div>
                ...
                >
                  <div className="flex items-center gap-2.5 truncate">
                    ...
                    <span className="truncate text-xs">
                      <strong className="text-white font-bold">[{e.code}]</strong> {e.mission_name}
                    </span>
                  </div>
        ...
        <div className="space-y-4 pt-2 border-t border-slate-800">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="block text-slate-300 font-bold">
                LEADER EMAIL <span className="text-cyan-400">*</span>
              </label>
              <input
                type="email"
                value={leader.email}
            ...
            <div className="space-y-1.5">
              <label className="block text-slate-300 font-bold">
                LEADER PHONE NUMBER <span className="text-cyan-400">*</span>
              </label>
              <input
                type="tel"
                value={leader.phone}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">
                  Each member gets their own wristband and food token.
                </p>
              </div>
              ...
              <div key={idx} className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-3 relative">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-white font-mono">
                    TEAM MEMBER {idx + 2} {idx < additionalSlotsRequired && <span className="text-cyan-400">(Required)</span>}
                  </span>
                  <button
                    type="button"
        ...
        <div className="pt-4">
          ...
          >
            {isSubmitting ? (
              <span>REGISTERING TEAM & ISSUING CREDENTIALS...</span>
            ) : (
              <>
                <span>COMPLETE REGISTRATION & GET TEAM PASSES</span>
